...
def plot_results_to_html(results_df, file_path, title="AUTOANALISIS"):
    ...
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Cosine Similarity", "ROUGE y BLEU"))

    # Graficar Cosine Similarity
    ...
    # Graficar ROUGE y BLEU en la segunda fila
    fig.add_trace(go.Bar(x=results_df['Pregunta'], y=results_df['ROUGE'], name='ROUGE'), row=2, col=1)
    fig.add_trace(go.Line(x=results_df['Pregunta'], y=results_df['BLEU'], name='BLEU'), row=2, col=1)
